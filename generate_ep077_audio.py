...
key = m.group(1)
script = (root/'episodes/ep077_script.txt').read_text()
paras = script.split('\n\n')
n = len(paras)
...
for i, part in enumerate(parts, 1):
    ...
    out = root/f'episodes/ep077_part{i}.mp3'
    out.write_bytes(r.content)
    print('saved', out, out.stat().st_size, flush=True)
concat = root/'episodes/ep077_concat.txt'
concat.write_text("file 'ep077_part1.mp3'\nfile 'ep077_part2.mp3'\nfile 'ep077_part3.mp3'\n")
subprocess.run(['ffmpeg','-y','-f','concat','-safe','0','-i',str(concat),'-c','copy',str(root/'audio/ep077.mp3')],check=True)
print('merged', root/'audio/ep077.mp3', (root/'audio/ep077.mp3').stat().st_size, flush=True)
