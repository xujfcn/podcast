...
ep = 550
title = 'EP550: Assurance Exception Expiry Recovery - Restore the Baseline Without Reviving Risk'
description = 'How AI API gateway teams can recover from expired exceptions without restoring stale authority or overwriting newer controls.'
pub_date = 'Wed, 16 Sep 2026 22:30:00 +0000'
parts = [part for part in (root / f'episodes/ep{ep:03d}_script.txt').read_text().split('\n\n') if part.strip()]
(root / 'audio').mkdir(exist_ok=True)

for index, part in enumerate(parts, 1):
    output = root / f'episodes/ep{ep:03d}_chunk{index}.mp3'
    command = ['edge-tts', '--voice', 'en-US-GuyNeural', '--text', part, '--write-media', str(output)]
...
concat.write_text(''.join(f"file 'ep{ep:03d}_chunk{index}.mp3'\n" for index in range(1, len(parts) + 1)))
...
enclosure.attrib.update(url=f'https://xujfcn.github.io/podcast/audio/ep{ep:03d}.mp3', length=str(audio.stat().st_size), type='audio/mpeg')
...
for tag, value in [('duration', duration), ('episode', str(ep)), ('episodeType', 'full'), ('explicit', 'false')]:
    ET.SubElement(item, f'{{{namespace}}}{tag}').text = value
