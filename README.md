# AI Dev Tools — The Crazyrouter Podcast

Your weekly breakdown of AI development tools, API gateways, model pricing, and building with GPT, Claude, Gemini, and DeepSeek.

🎧 **Subscribe:** [RSS Feed](https://xujfcn.github.io/podcast/feed.xml)

## Episodes

### EP006: Building an AI Chatbot in 10 Minutes with One API Key
*March 15, 2026 · 4 min*

Building an AI chatbot is easier than most tutorials make it look. This episode breaks down the simplest architecture: one API key, one compatible endpoint, and a thin backend you can ship fast.

🔗 [Listen (MP3)](https://xujfcn.github.io/podcast/audio/ep006.mp3)

---

### EP001: AI API Gateway Explained — Your Questions Answered
*March 11, 2026 · 3 min*

What is an AI API gateway? How many models does Crazyrouter support? Is it cheaper than going direct? We answer the 9 most common questions.

🔗 [Listen (MP3)](https://xujfcn.github.io/podcast/audio/ep001.mp3)

---

## About

**Crazyrouter** — One API key, 627+ AI models, lower than official pricing.

- 🌐 [Website](https://crazyrouter.com)
- 📖 [Docs](https://docs.crazyrouter.com)
- 💬 [Telegram](https://t.me/crzrouter)
- 🐙 [GitHub](https://github.com/xujfcn)
