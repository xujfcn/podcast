...
key = m.group(1)
script = (root/'episodes/ep076_script.txt').read_text()
paras = script.split('\n\n')
n = len(paras)
...
for i, part in enumerate(parts, 1):
    ...
    out = root/f'episodes/ep076_part{i}.mp3'
    out.write_bytes(r.content)
    print('saved', out, out.stat().st_size, flush=True)
concat = root/'episodes/ep076_concat.txt'
concat.write_text("file 'ep076_part1.mp3'\nfile 'ep076_part2.mp3'\nfile 'ep076_part3.mp3'\n")
subprocess.run(['ffmpeg','-y','-f','concat','-safe','0','-i',str(concat),'-c','copy',str(root/'audio/ep076.mp3')],check=True)
print('merged', root/'audio/ep076.mp3', (root/'audio/ep076.mp3').stat().st_size, flush=True)
